...
const gridStyle = computed(() => ({
  display: 'grid',
  gridTemplateColumns: `repeat(${gameStore.N}, 15px)`,
  gap: '5px',
}));
...
// });
const blockArray = computed(() => {
  return gameStore.gameStarted ? gameStore.realArray : Array.from({ length: gameStore.N }, () => Array(gameStore.N).fill(-1));
});
</script>
...
<template>
  <div class="integration-area">
    ...
    <div class="info-area">
      <div class="info-item">
        <span>雷数: {{ gameStore.NM }}</span>
      </div>
      <div class="info-item">
        <span>时间(秒): {{ gameStore.timer }} </span>
      </div>
    </div>
  ...
  <div v-for="(num, j) in row" :key="`${i}-${j}`">
    <BasicBlock 
      :Num="num"
      :Posarray="gameStore.rePosArray"
      :Pos="{ x: i, y: j }"
      @gameStart="gameStore.startGame"
      @zeroMessage="gameStore.handleZeroMessage"
      @nineMessage="gameStore.handleNineMessage"
      @updatePosition="gameStore.updatePosition"
    />
  </div>
</div>
